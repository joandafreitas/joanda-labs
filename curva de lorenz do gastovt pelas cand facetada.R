...
ggplot(df_lorenz, aes(x = prop_municipios, y = prop_gastovt, color = genero)) +
  ...
  labs(
    title = "",
    x = "Municípios organizados em ordem crescente do gasto por voto mediano",
    y = "Distribuição do gasto por voto mediano por candidatos e candidatas nas regiões da Paraíba",
    color = ""
  ) +
  theme_minimal() +
  theme(
