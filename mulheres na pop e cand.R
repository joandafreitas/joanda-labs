...
resumo <- database_candidatos_candidatas %>%
  ...
  summarise(
    ...
    mediana_fem_dos_hab = median(`%fem_dos_hab`, na.rm = TRUE),
    mad_fem_dos_hab = mad_robusta(`%fem_dos_hab`)
  ) %>%
  pivot_longer(
    cols = -reg,
  ...
  mutate(
    ...
    variavel = recode(variavel,
                      cand_fem = "% candidatas",
                      fem_dos_hab = "% mulheres na população"),
    variavel = factor(variavel, levels = c("% mulheres na população", "% candidatas"))
  )
...
ggplot(resumo, aes(x = reg, y = mediana, fill = variavel)) +
  ...
  labs(
    title = "",
    x = "Região",
    y = "Percentual mediano de mulheres na população e de candidatas (%)",
    fill = ""
  ) +
