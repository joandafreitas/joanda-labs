...
orientação_sexual_região_mata <- orientação_sexual_região %>%
  filter(reg == "mata") %>%
  mutate(id_gen = factor(ori_sex, levels = unique(ori_sex)))

# 4. Gráfico só para a região agreste
ggplot(orientação_sexual_região_mata, aes(x = ori_sex, y = n, fill = ori_sex)) +
  ...
  expand_limits(y = max(orientação_sexual_região_mata$n) * 1.15)