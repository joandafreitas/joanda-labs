...
orientação_sexual_região_borborema <- orientação_sexual_região %>%
  filter(reg == "borborema") %>%
  mutate(id_gen = factor(ori_sex, levels = unique(ori_sex)))

# 4. Gráfico só para a região agreste
ggplot(orientação_sexual_região_borborema, aes(x = ori_sex, y = n, fill = ori_sex)) +
  ...
  expand_limits(y = max(orientação_sexual_região_borborema$n) * 1.15)